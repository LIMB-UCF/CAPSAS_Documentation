...
class EmgRawDataConfig:
    ...
    batch_len: int = 32 #cange back to 32
...
class Gforce:
    ...
    def _on_data_response(self, q: Queue, bs: bytearray):
        ...
        match data_type: #check here maybe modify
...
def init_lsl():
    srate = 500
    name = 'OyMotion GForce+'
    emg_type = 'EMG'
    n_channels = 8 
    info = StreamInfo(name, emg_type, n_channels, srate, 'float32') 
    return info
mode = 'push_sample' # 'push_sample' or 'push_chunk'
async def main():
    gforce = Gforce()
    ...
    srate = 500
    info = init_lsl() #initialize lsl stream
    #appending additional metadata to the stream
    ...
    hw_delay = 0
    sent_samples = 0
    queue = Queue(maxsize = 5*srate) #create a buffer of 5 seconds of data
        ...
        try:
            if mode == 'push_sample':
                for e in await q.get():
                    ...
                    sent_samples += 1
                    #print(f'data sent: {sent_samples}')
            elif mode == 'push_chunk':
                for e in await q.get():
                    await queue.put(e.tolist())
                    print(queue.qsize())
                elapsed_time = local_clock() - start_time
                required_samples = int(srate * elapsed_time) - sent_samples
                print(f'required_samples are {required_samples} and queue size is {queue.qsize()}')
                if required_samples > 0 and queue.qsize() >= required_samples:  
                    print('enter the loop') 
                    mychunk = []
                    for i in range(required_samples):
                        mychunk.append(await queue.get())
                    stamp = local_clock() - hw_delay 
                    outlet.push_chunk(mychunk, stamp)
                    print(mychunk)
                    sent_samples += required_samples  
                    print('sent samples:', sent_samples)
            # for e in await q.get():
            #     sample = e.tolist()
            #     timestamp = local_clock()
            #     outlet.push_sample(sample, timestamp)
            #     sent_samples += 1
            #     print(f'Sent sample: {sample} and data sent: {sent_samples}')
